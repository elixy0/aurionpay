...
function buildMerchantAuth(cartContents) {
  ...
  return jwt.sign(payload, privateKeyPem, { algorithm: "ES256K" });
}

async function createHSPOrder({
  orderId,
  paymentRequestId,
  amount,
  currency,
  ...
  if (response.data.code !== 0) {
    throw new Error(`HSP error: ${response.data.msg}`);
  }

  return response.data.data;
}

async function queryHSPPayment(cartMandateId) {
...
export { createHSPOrder, queryHSPPayment, TOKENS };